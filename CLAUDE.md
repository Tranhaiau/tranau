...
- Thư mục `subagents/` trong mỗi skill (`pgs-sales-analytics`, `pgs-service-analytics`, `pgs-training-management`) là **spec chi tiết**, không phải agent thật. 11 sub-agent đó đã được đăng ký thật dưới dạng Custom Agent tại `.claude/agents/*.md` (context riêng, tool giới hạn, không có quyền spawn agent khác) — dùng Agent tool với đúng `subagent_type` khi cần delegate, xem bảng ánh xạ ở Layer 4 trong từng SKILL.md.
